...
The model combines a pretrained ResNet-18 (a type of convolutional neural network that’s great at recognizing visual patterns) with a linear regression layer that maps the network’s output to predicted fMRI responses. The model was trained on BOLD5000, a dataset of fMRI responses from three subjects viewing 4,916 images (I wonder what the other 84 images were that they couldn’t include). 

Even though our brains are messy spaghetti codes, visual processing is surprisingly consistent across people—which makes this possible. Because our brains process visuals in nearly identical ways, this tool is awesome for cognitive modeling!
